---
mode: agent
description: Review pending Go changes against repo style and the prime directives. Reports issues; does not edit.
---

Target: ${input:target:File or diff to review (leave empty for staged + unstaged changes)}

Review pass, in this order. Report findings as a numbered list. Do not edit unless asked.

## 1. Scope and bloat
- Does the diff touch only what the task required?
- Any new files, packages, or interfaces? Was each one necessary, or could the change have lived in an existing file?
...
For each issue: file, line range, what's wrong, the smallest fix. No fixes applied unless requested.
